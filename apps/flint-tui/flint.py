...
def text_key(key):
    ...
    specials = (tui.KEY_ESCAPE, tui.KEY_UP, tui.KEY_DOWN, tui.KEY_LEFT,
                tui.KEY_RIGHT, getattr(tui, "KEY_PAGE_UP", -1),
                getattr(tui, "KEY_PAGE_DOWN", -1), getattr(tui, "KEY_HOME", -1),
                getattr(tui, "KEY_END", -1), getattr(tui, "KEY_DELETE", -1))
...
def draw_header(width, title, subtitle="", right=""):
    tui.addstr(0, 0, " " * width)
    tui.addstr(1, 0, " " * width)
    heading = " " + title
    if right:
        room = max(1, width - len(right) - 1)
        heading = clip(heading, room)
        heading = heading + " " * max(0, room - len(heading)) + right
    tui.addstr(0, 0, clip(heading, width), tui.INVERSE)
    if subtitle:
        tui.addstr(1, 0, clip(subtitle, width), tui.BOLD)


def draw_footer(width, height, text):
    tui.addstr(height - 1, 0, " " * width)
    tui.addstr(height - 1, 0, clip(text, width), tui.INVERSE)
...
def draw_message(width, height, title, body, footer="Press any key"):
    ...
    visible = max(1, height - HEADER_H - FOOTER_H)
    row = HEADER_H
...
def choose(width, height, title, options, selected=0, subtitle="",
    ...
    while not solaros.should_exit():
        ...
        def draw_option(index):
            row = HEADER_H + index - start
            tui.addstr(row, 0, " " * width)
            attr = tui.INVERSE if index == selected else tui.NORMAL
            tui.addstr(row, 0, clip(options[index], width), attr)
        ...
        if previous_start != start:
            ...
            if previous is not None and start <= previous < start + visible:
                draw_option(previous)
            draw_option(selected)
        ...
        if key in hotkeys:
            ...
            return -key - 1
        if key in (tui.KEY_ESCAPE, ord("q"), ord("Q")):
            return None
        if key in (tui.KEY_UP, ord("k")):
            selected = (selected - 1) % len(options)
        elif key in (tui.KEY_DOWN, ord("j")):
            selected = (selected + 1) % len(options)
        elif key == getattr(tui, "KEY_PAGE_UP", -1001):
            selected = max(0, selected - visible)
        elif key == getattr(tui, "KEY_PAGE_DOWN", -1002):
            selected = min(len(options) - 1, selected + visible)
        elif key in (KEY_ENTER, KEY_LF, tui.KEY_RIGHT):
            return selected
    return None
...
def progress(width, height, title, current, total, detail=""):
    ...
    filled = max(0, min(bar_width, bar_width * current // total)) if total else 0
    tui.addstr(max(HEADER_H, height // 2), 1,
...
def draw_reader(width, height, item, lines, scroll, favorite, external=False):
    ...
    for line in lines[scroll:scroll + visible]:
        value, style = line
        attr = tui.BOLD if style in ("heading", "bold") else tui.NORMAL
        tui.addstr(row, 0, raw_clip(value, width) if style == "mono" else clip(value, width), attr)
        row += 1
...
def cursor_vertical(value, cursor, direction):
...
def line_buffer(text, limit=MAX_NOTE_BYTES):
    """Create the editor model; each list entry is one physical Markdown line."""
    return clean_document(text, limit).split("\n")


def line_buffer_text(lines):
    return "\n".join(lines)


def draw_buffer_row(width, height, lines, position, top, cursor_row,
                    cursor_col, horizontal, caret=False):
    """Draw one editor row without allocating a closure for every keypress."""
    screen_row = HEADER_H + position - top
    if screen_row < HEADER_H or screen_row >= height - FOOTER_H:
        return
    tui.addstr(screen_row, 0, " " * width)
    offset = horizontal if position == cursor_row else 0
    tui.addstr(screen_row, 0, raw_clip(lines[position][offset:], width))
    if caret:
        screen_col = cursor_col - offset
        under = (lines[cursor_row][cursor_col]
                 if cursor_col < len(lines[cursor_row]) else " ")
        tui.putch(screen_row, screen_col, ord(under), tui.INVERSE)


def edit_document(width, height, initial, limit=MAX_NOTE_BYTES, title="Edit note"):
    lines = line_buffer(initial, limit)
    cursor_row = len(lines) - 1
    cursor_col = len(lines[cursor_row])
    preferred_column = None
    length = utf8_size(line_buffer_text(lines))
    previous_cursor_row = None
    previous_top = None
    structural_change = True
    while not solaros.should_exit():
        visible = max(1, height - HEADER_H - FOOTER_H)
        top = max(0, min(cursor_row - visible // 2, len(lines) - visible))
        horizontal = max(0, cursor_col - width + 1)

        full = structural_change or previous_top != top
        if full:
            tui.clear()
            for position in range(top, min(len(lines), top + visible)):
                draw_buffer_row(width, height, lines, position, top, cursor_row,
                                cursor_col, horizontal, position == cursor_row)
            draw_footer(width, height, "Ctrl+S save  Ctrl+B vault  Esc cancel")
        else:
            if previous_cursor_row is not None and previous_cursor_row != cursor_row:
                draw_buffer_row(width, height, lines, previous_cursor_row, top,
                                cursor_row, cursor_col, horizontal)
            draw_buffer_row(width, height, lines, cursor_row, top, cursor_row,
                            cursor_col, horizontal, True)
        draw_header(width, title, "Line {}/{}".format(cursor_row + 1, len(lines)),
                    "{}/{}".format(length, limit))
        tui.refresh()
        ...
        key = wait_key()
        if key == tui.KEY_ESCAPE:
            return "cancel", line_buffer_text(lines)
        if key == KEY_CTRL_S:
            return "save", line_buffer_text(lines)
        if key == KEY_CTRL_B:
            return "browse", line_buffer_text(lines)
        current = lines[cursor_row]
        if key in (KEY_ENTER, KEY_LF) and length < limit:
            ...
            if cursor_col:
                length -= utf8_size(current[cursor_col - 1:cursor_col])
                ...
                lines[cursor_row - 1] = previous + current
                del lines[cursor_row]
                cursor_row -= 1
                length -= 1
                structural_change = True
                preferred_column = None
        elif key == KEY_DELETE or key == getattr(tui, "KEY_DELETE", -1003):
            if cursor_col < len(current):
                length -= utf8_size(current[cursor_col:cursor_col + 1])
            ...
            if preferred_column is None:
                preferred_column = cursor_col
            cursor_row = max(0, cursor_row - 1)
            cursor_col = min(preferred_column, len(lines[cursor_row]))
            ...
            cursor_row = max(0, cursor_row - visible)
            if preferred_column is None:
                preferred_column = cursor_col
            cursor_col = min(preferred_column, len(lines[cursor_row]))
        elif key == getattr(tui, "KEY_PAGE_DOWN", -1002):
            cursor_row = min(len(lines) - 1, cursor_row + visible)
            if preferred_column is None:
                preferred_column = cursor_col
            cursor_col = min(preferred_column, len(lines[cursor_row]))
        elif key == getattr(tui, "KEY_HOME", -1004):
            cursor_col = 0
            preferred_column = None
        elif key == getattr(tui, "KEY_END", -1005):
            cursor_col = len(current)
            preferred_column = None
        elif key == 9 and length <= limit - 4:
...
def note_screen(width, height, path, index, root, initial_heading=""):
    ...
    while not solaros.should_exit():
        ...
        if key in (tui.KEY_ESCAPE, tui.KEY_LEFT, ord("q"), ord("Q")):
            return None
        if key in (tui.KEY_UP, ord("k")):
        ...
        if key not in (tui.KEY_UP, tui.KEY_DOWN, ord("j"), ord("k"),
                       getattr(tui, "KEY_PAGE_UP", -1001),
                       getattr(tui, "KEY_PAGE_DOWN", -1002)):
            gc.collect()
    return None
...
def main():
    ...
    if source is not None or list_path is not None:
        ...
        return
    try:
        height, width = tui.size()
        if width < MIN_WIDTH or height < MIN_HEIGHT:
            tui.clear()
            tui.addstr(0, 0, clip("Flint TUI needs a 24x8 terminal.", width))
            tui.refresh()
            wait_key()
            return
        path = argument_path()
